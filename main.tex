...
Thank you to Zo\"{e} Batterman, Vincent Costa, and Gabe Ong for offering suggestions and changes to the notes. Thanks to Sidhanth Raman for helpful correspondence in preparing this class.
...
Let $\mathcal{U} = \left\{ U_i \right\}_{i\in I}$ be a cover of $X$, let $F$ be a sheaf, and define
...
We define the \textit{Cech cohomology} of $F\in \Ab(X)$ over the cover $\mathcal{U}$ to be
\begin{align*}
...
\begin{exercise} If $X$ is a space, $F\in \Ab(X)$, and $\mathcal{U},\mathcal{V}$ are open covers of $X$ for which $\mathcal{V}$ is a refinement of $\mathcal{U}$, define a natural restriction morphism
\begin{align*}
...
We define the \emph{\v{C}ech cohomology} of $F\in \Ab(X)$ by
\begin{align*}
...
\begin{exercise} Give an example of \emph{noise} in \v{C}ech cohomology. That is, find a cover which creates a cohomology class which vanishes under refinement.
\end{exercise}



\begin{proposition}\label{prop:Cech-0-is-global-sections} 
For any space $X$ and any $F\in\Ab(X)$, we have a canonical isomorphism
\begin{align*}
...
\end{align*}
is a short exact sequence of sheaves in $\Ab(X)$, it induces a long exact sequence
\begin{align*}
...
\end{align*}
\end{proposition}
\begin{proof}[Sketch] We first show, for any cover $\mathcal{U}$, that $\check{C}^q(\mathcal{U},-)$ is an exact functor on $\Ab(X)$. A diagram chase then implies we get a long exact sequence on $\check{H}^\ast(\mathcal{U},-)$. Then we take a colimit over all $\mathcal{U}$ in the filtered category of covers $\Cov(X)$, and use that filtered colimits preserve exactness to conclude (\Cref{exa:CovX-filtered} and \Cref{cor:filtered-colimits-preserve-exactness}). The details are spelled out in \cite[pp.~28--29]{Hirzebruch}.
\end{proof}

\begin{remark} The definition of \v{C}ech cohomology in \Cref{def:Cech-cohomology} doesn't require $F$ to be a sheaf --- it makes sense for $F$ any presheaf. In particular we didn't use the sheaf condition in setting up this theory, aside from using \emph{gluing} to argue that $\check{H}^0(X,F) \cong \Gamma(X,F)$. We can make sense of \v{C}ech cohomology in any \emph{presheaf}, which begs the question: when do the \v{C}ech cohomology of a sheaf $F$ and of its sheafification $\til{F}$ agree?
\end{remark}
...
\end{proposition}
\begin{proof} This is because the stalk $F_x$ must be zero.
\end{proof}
...
\begin{theorem} If $X$ is paracompact, and $F$ is a presheaf with corresponding sheaf $\til{F}$, then the natural map $\check{H}^q(X,F) \to \check{H}^q(X,\til{F})$ is an isomorphism for all $q\ge0$.
\end{theorem}
\begin{proof} Consider the sheafification map $F \to F^\sharp$ and take its kernel and cokernel
\begin{align*}
    0 \to K \to F \to F^\sharp \to C \to 0.
\end{align*}
As $F \to F^\sharp$ sheafifies to an isomorphism, and since sheafification is an exact functor, we have that $K$ and $C$ sheafify to zero. Hence their higher cohomology vanishes by \Cref{lem:presheaf-sheafifying-to-zero-has-no-cohomology}, and using the long exact sequence on cohomology attached to short exact sequences of sheaves, we obtain that the induced maps $\check{H}^q(X,F) \to \check{H}^q(X,F^\sharp)$ are all isomorphisms.
\end{proof}

So over paracompact spaces, there is no difference between the cohomology of a presheaf and of its resulting sheafification.
...
\begin{proposition} Filtered colimits valued in $\Set$ or $\Ab$ or any other reasonable concrete category (see \href{https://math.stackexchange.com/a/2143601}{here} for details for instance) commute with finite limits.
\end{proposition}
...
An interesting question in the setting of abelian categories is the interaction between exactness and filtered colimits. Indeed exactness of filtered colimits is a consequence of Grothendieck's axiom AB5 for abelian categories \cite[1.8.1]{Tohoku}.
...
We leverage \Cref{cor:filtered-colimits-preserve-exactness} a handful of times in the notes. It is important to see when categories are (co)filtered, since this will be useful. A very obvious example is the category $\Open(X)$, which is cofiltered, hence $\Open(X)^\op$ is filtered.
